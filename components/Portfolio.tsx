...
const Portfolio: React.FC = () => {
    ...
    return (
        <section id="portfolio" className="relative w-full min-h-screen bg-[#fcf4ee] py-24 pb-48">
            ...
            <div className="container mx-auto px-4 md:px-8 relative z-10">
                ...
                <div className="mb-16 flex flex-col items-center justify-center text-center">
                    <h2 className="text-5xl md:text-7xl font-black uppercase text-black tracking-tighter mb-4">
                        Media <span className="text-[#5862ec]">Vault</span>
                    </h2>
                    <div className="flex gap-2 items-center text-gray-400 text-sm font-mono tracking-widest uppercase">
                        <div className="w-2 h-2 bg-[#5862ec] rounded-full animate-pulse" />
                ...
                {selectedItem && (
                    ...
                    >
                        ...
                        <button
                            ...
                            className="absolute top-8 right-8 z-50 p-4 bg-white/10 hover:bg-[#5862ec] rounded-full group transition-all"
                        >
                            <X className="text-black group-hover:text-black" size={24} />
                        </button>
                        ...
                        <div className="mt-8 text-center">
                            <h3 className="text-3xl font-black uppercase text-black mb-2">{selectedItem.title}</h3>
                            <div className="flex items-center justify-center gap-4">
                                <span className="text-[#5862ec] font-mono text-sm uppercase px-3 py-1 border border-[#5862ec] rounded-full">
...
const FilterTab = ({ label, icon, isActive, onClick }: { label: string, icon: React.ReactNode, isActive: boolean, onClick: () => void }) => (
    <button
        ...
        className={`flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold uppercase tracking-wide transition-all ${isActive
            ? 'bg-[#5862ec] text-black shadow-[0_0_15px_rgba(255,204,0,0.4)] scale-105'
        ...
        >
            <div className="relative aspect-video overflow-hidden rounded-lg border border-white/10 bg-gray-900">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-16 h-16 md:w-20 md:h-20 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center border border-white/20 group-hover:scale-110 group-hover:bg-[#5862ec] transition-all duration-300">
                        {item.youtubeId ? (
            ...
            <div className="mt-4 flex justify-between items-end">
                <div>
                    <h3 className="text-2xl font-bold text-black uppercase leading-none mb-1 group-hover:text-[#5862ec] transition-colors">{item.title}</h3>
                    <p className="text-gray-500 text-xs font-mono uppercase tracking-widest">YouTube / Documentary</p>
                </div>
                <div className="w-8 h-[1px] bg-white/20 group-hover:w-16 group-hover:bg-[#5862ec] transition-all" />
            </div>
        </motion.div>
    );
};
        ...
        >
            ...
            {item.stats && (
                <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 flex items-center gap-2 z-20">
                    <TrendingUp size={12} className="text-[#5862ec]" />
            ...
            <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black via-black/80 to-transparent z-20 translate-y-2 group-hover:translate-y-0 transition-transform">
                ...
                <p className="text-[10px] text-[#5862ec] uppercase tracking-wider font-bold">{item.client}</p>
            </div>
        </motion.div>
    );
};
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20">
                <span className="text-[#5862ec] font-black text-4xl tracking-tighter uppercase mix-blend-difference">View</span>
            </div>
